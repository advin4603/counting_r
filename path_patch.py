...
class Node:
    ...
    def __init__(
        ...
        if "hook_" in node_name:
            ...
            if self.layer is not None:
                self.layer = int(self.layer.group(1))
        else:
        ...
        if head is not None:
            assert isinstance(
                head, int), f"head should be an int, not {type(head)}"

        if kv_head is not None:
            assert isinstance(
                kv_head, int), f"head should be an int, not {type(kv_head)}"

        if neuron is not None:
            assert isinstance(
                neuron, int), f"neuron should be an int, not {type(neuron)}"
        self.head = head or kv_head
        self.neuron = neuron
    ...
    def get_posn(self):
        ...
        if self.layer is None:
            layer = - \
                1 if self.component_name in [
                    "embed", "pos_embed", "tokens"] else 10e5
        else:
            layer = self.layer
        ...
        if "resid_pre" in self.activation_name:
            ...
            raise Exception(
                "Error in function - haven't yet added a case for this name.")
    ...
    def check_sender(self, model: HookedTransformer) -> List["Node"]:
        ...
        assert self.activation_name in model.hook_dict.keys(
        ), f"Error: node '{self.activation_name}' is not in the hook dictionary."

        # Do main validity check
        # TODO - allow "result", since there are use cases (e.g. if you're editing the new_cache directly)
        valid_sender_nodes = ["z", "attn_out", "post",
                              "mlp_out", "resid_pre", "resid_post", "resid_mid"]
    ...
    def check_and_split_receiver(self, model: HookedTransformer) -> List["Node"]:
        ...
        # Do main validity check
        valid_receiver_nodes = ["q", "k", "v", "pattern", "q_input",
                                "k_input", "v_input", "resid_pre", "resid_mid", "resid_post", "pre"]
        ...
        def hook_fn(activations: Float[Tensor, "..."], hook: HookPoint) -> Float[Tensor, "..."]:
            ...
            if self.head is not None:
                ...
                if "pattern" in hook.name or "score" in hook.name:
                    assert isinstance(
                        seq_pos_indices, slice), "Can't patch attention patterns/scores at specific sequence positions (ambiguous whether this is query or key positions)."
...
class IterNode:
    ...
    def __init__(
        ...
        self.component_names = [node_names] if isinstance(
            node_names, str) else node_names
        ...
        for node in self.component_names:
            ...
            if node in ["q", "z", "pattern"]:
                self.shape_names[node].append("head")
            if node in ["k", "v"]:
                self.shape_names[node].append("kv_head")
            if node in ["pre", "post"]:
                self.shape_names[node].append("neuron")
    ...
    def get_node_dict(
        ...
        shape_values_all = {"seq_pos": seq_len, "layer": model.cfg.n_layers,
                            "head": model.cfg.n_heads, "neuron": model.cfg.d_mlp, "kv_head": model.cfg.n_key_value_heads}  # patch for gqa
        ...
        if self.seq_pos == "each":
            seq_pos_indices = list(einops.repeat(
                t.arange(seq_len), "s -> s b 1", b=batch_size))
        ...
        for node_name, shape_names in self.shape_names.items():
            ...
            shape_values_dict = {
                name: value for name, value in shape_values_all.items() if name in shape_names}
            shape_ranges = {name: range(
                value) for name, value in shape_values_dict.items() if name != "seq_pos"}
            ...
            shape_values_list = product_with_args_kwargs(
                seq_pos_indices, **shape_ranges)
            self.nodes_dict[node_name] = [
                (args[0], Node(node_name, **kwargs)) for args, kwargs in shape_values_list]
...
def get_batch_and_seq_pos_indices(seq_pos, batch_size, seq_len):
    ...
    if seq_pos is None:
        ...
        assert (seq_pos.ndim == 2) and (seq_pos.shape[0] == batch_size) and (
            seq_pos.shape[1] <= seq_len) and (seq_pos.max() < seq_len), "Invalid 'seq_pos' argument."
        seq_sub_pos_len = seq_pos.shape[1]
        seq_pos_indices = seq_pos
        batch_indices = einops.repeat(t.arange(
            batch_size), "batch -> batch seq_sub_pos", seq_sub_pos=seq_sub_pos_len)
...
def _path_patch_single(
    ...
    assert isinstance(_sender_nodes, list) and isinstance(
        _receiver_nodes, list)
    ...
    if direct_includes_mlps or any([n.component_name == "pre" for n in receiver_nodes]):
        ...
        hooks_for_freezing.append((lambda name: name.endswith(
            "z"), partial(hook_fn_generic_patching, cache=orig_cache)))
        if not direct_includes_mlps:
            hooks_for_freezing.append((lambda name: name.endswith(
                "post"), partial(hook_fn_generic_patching, cache=orig_cache)))

        # Get all the hooks we need for patching heads (and possibly MLPs)
        for node in sender_nodes:
            hooks_for_patching_senders.append((node.activation_name, node.get_patching_hook_fn(
                new_cache, batch_indices, seq_pos_indices)))

        # Get all the hooks we need for caching receiver nodes
        for node in receiver_nodes:
            hooks_for_caching_receivers.append((node.activation_name, partial(
                hook_fn_generic_caching, name="receiver_activations")))
        ...
        model.run_with_hooks(
            ...
            hooks_for_freezing + hooks_for_patching_senders,
        ...
        for sender_node in sender_nodes:
            ...
            diff = new_cache[sender_node.activation_name] - \
                orig_cache[sender_node.activation_name]
            ...
            if sender_node.component_name == "post":
                neuron_slice = slice(None) if sender_node.neuron is None else [
                    sender_node.neuron]
                diff = einops.einsum(
                    diff[..., neuron_slice], model.W_out[sender_node.layer,
                                                         neuron_slice],
                    "batch pos d_mlp, d_mlp d_model -> batch pos d_model"
                )
            # If it's the "z" part of attn heads, we map through W_O (maybe just taking one head)
            elif sender_node.component_name == "z":
                head_slice = slice(None) if sender_node.head is None else [
                    sender_node.head]
                diff = einops.einsum(
                    diff[..., head_slice,
                         :], model.W_O[sender_node.layer, head_slice],
            ...
            for i, receiver_node in enumerate(receiver_nodes):
                ...
                if receiver_node.component_name in ["q", "k", "v", "q_input", "k_input", "v_input"]:
                    ...
                    if receiver_node.component_name in ["q", "k", "v"]:
                        receiver_node = Node(
                            f"{receiver_node.component_name}_input", layer=receiver_node.layer, head=receiver_node.head)
                ...
                if len(model.hook_dict[receiver_node.activation_name].ctx) == 0:
                    model.hook_dict[receiver_node.activation_name].ctx["receiver_activations"] = t.zeros_like(
                        orig_cache[receiver_node.activation_name])
                    model.add_hook(
                        receiver_node.activation_name,
                        partial(hook_fn_generic_patching_from_context,
                                name="receiver_activations", add=True),
                        level=1
                    )

                if receiver_node.component_name in ["q_input", "k_input", "v_input"]:
                    head_slice = slice(None) if (receiver_node.head is None) else [
                        receiver_node.head]
                    ...
                    model.hook_dict[receiver_node.activation_name].ctx["receiver_activations"][batch_indices,
                                                                                               seq_pos_indices, head_slice] += diff
...
def path_patch(
    ...
    new_input: Optional[Union[str, List[str],
                              Int[Tensor, "batch seq_len"]]] = None,
    sender_nodes: Union[IterNode, Node, List[Node]] = [],
    receiver_nodes: Union[IterNode, Node, List[Node]] = [],
    patching_metric: Union[Callable,
                           Literal["loss", "loss_per_token"]] = "loss",
    ...
    assert not all([isinstance(receiver_nodes, IterNode), isinstance(
        sender_nodes, IterNode)]), "Can't iterate over both senders and receivers!"

    # Check other arguments
    assert any([isinstance(new_cache, ActivationCache), new_cache ==
               "zero", new_cache is None]), "Invalid new_cache argument."
    # assert (new_input is not None) or (new_cache == "zero"), "If new_cache is not 'zero' then you must provide new_input."
    if isinstance(patching_metric, str):
        assert patching_metric in [
            "loss", "loss_per_token"], "Invalid patching_metric argument."
    assert not (isinstance(patching_metric, str)
                and apply_metric_to_cache), "Can't apply metric to cache if metric is 'loss' or 'loss_per_token'."
    ...
    if new_cache == "zero":
        new_cache = ActivationCache({k: t.zeros_like(v)
                                    for k, v in orig_cache.items()}, model=model)
    elif new_cache is None:
        _, new_cache = model.run_with_cache(
            new_input, return_type=None, names_filter=relevant_names_filter)
    ...
    if isinstance(receiver_nodes, IterNode):
        receiver_nodes_dict = receiver_nodes.get_node_dict(
            model, new_cache["q", 0])
        progress_bar = tqdm(total=sum(len(node_list)
                            for node_list in receiver_nodes_dict.values()))
        for receiver_node_name, receiver_node_list in receiver_nodes_dict.items():
            progress_bar.set_description(
                f"Patching over {receiver_node_name!r}")
            results_dict[receiver_node_name] = []
            for (seq_pos, receiver_node) in receiver_node_list:
                results_dict[receiver_node_name].append(path_patch_single(
                    sender=sender_nodes, receiver=receiver_node, seq_pos=seq_pos))
        ...
        return {
            node_name: t.tensor(results).reshape(list(receiver_nodes.shape_values[node_name].values(
            ))) if isinstance(results[0], float) else results
    ...
    # If we're fixing receiver(s), and iterating over senders:
        ...
        sender_nodes_dict = sender_nodes.get_node_dict(
            model, new_cache["q", 0])
        progress_bar = tqdm(total=sum(len(node_list)
                            for node_list in sender_nodes_dict.values()))
        for sender_node_name, sender_node_list in sender_nodes_dict.items():
            ...
            for (seq_pos, sender_node) in sender_node_list:
                results_dict[sender_node_name].append(path_patch_single(
                    sender=sender_node, receiver=receiver_nodes, seq_pos=seq_pos))
                progress_bar.update(1)
                t.cuda.empty_cache()
        progress_bar.close()
        for node_name, node_shape_dict in sender_nodes.shape_values.items():
            if verbose:
                print(
                    f"results[{node_name!r}].shape = ({', '.join(f'{s}={v}' for s, v in node_shape_dict.items())})")
        return {
            node_name: t.tensor(results).reshape(list(sender_nodes.shape_values[node_name].values(
            ))) if isinstance(results[0], float) else results
            for node_name, results in results_dict.items()
        }
...
def _act_patch_single(
    ...
    for node in patching_nodes:
        batch_indices, seq_pos_indices = get_batch_and_seq_pos_indices(
            node.seq_pos, batch_size, seq_len)
        model.add_hook(
            node.activation_name,
            node.get_patching_hook_fn(
                new_cache, batch_indices, seq_pos_indices)
...
def act_patch(
    ...
    assert (new_input is not None) or (
        new_cache is not None), "Must specify either new_input or new_cache."
    if isinstance(patching_metric, str):
    ...
    if new_cache == "zero":
        _, cache = model.run_with_cache(orig_input, return_type=None)
        new_cache = ActivationCache({k: t.zeros_like(v)
                                    for k, v in cache.items()}, model=model)
    elif new_cache is None:
    ...
    for node_name, node_list in nodes_dict.items():
        ...
        for (seq_pos, node) in node_list:
            ...
            results_dict[node_name].append(
                act_patch_single(patching_nodes=node))
            progress_bar.update(1)
            t.cuda.empty_cache()
    progress_bar.close()
    for node_name, node_shape_dict in patching_nodes.shape_values.items():
        if verbose:
            print(
                f"results[{node_name!r}].shape = ({', '.join(f'{s}={v}' for s, v in node_shape_dict.items())})")
    return {
        node_name: t.tensor(results).reshape(list(patching_nodes.shape_values[node_name].values(
        ))) if isinstance(results[0], float) else results
        for node_name, results in results_dict.items()
    }
