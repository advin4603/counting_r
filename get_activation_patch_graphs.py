...
# %%
model_id = MODEL_NAME
model = transformer_lens.HookedTransformer.from_pretrained(model_name=model_id, device="cpu")
...
def get_incomplete_tokens(incomplete_example: str, example_answer: str, model: HookedTransformer) -> tuple[torch.Tensor, torch.Tensor]:
    incomplete_tokens = model.to_tokens(incomplete_example + example_answer, prepend_bos=True)[:, :-1]
...
def get_incomplete_str_tokens(incomplete_example: str, example_answer: str, model: HookedTransformer) -> list[str]:
    incomplete_tokens = model.to_str_tokens(incomplete_example + example_answer, prepend_bos=True)[:-1]
    return incomplete_tokens
...
def get_logits(incomplete_example: str, answer_set: list[str], model: HookedTransformer) -> tuple[Float[np.ndarray, "answer layer"], list[str]]:
    ...
    answer_tokens = model.to_tokens([incomplete_example + answer for answer in answer_set], prepend_bos=True)[:, -1]
...
while True:
    ...
    if logit_diff < LOGIT_DIFF_CUTOFF:
        continue
    token_count = model.to_tokens(prompt, prepend_bos=True).shape[1]
    negative_done = set()
    ...
    while True:
        ...
        for _ in range(NEGATIVE_SAMPLE_BATCH_SIZE):
            ...
            if model.to_tokens(negative_word_prompt, prepend_bos=True).shape[1] != token_count:
                continue
